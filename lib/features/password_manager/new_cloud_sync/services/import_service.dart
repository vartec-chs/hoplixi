...
class ImportDropboxService {
  ...
  Future<ServiceResult<CloudVersionInfo?>> checkForNewVersion({
    ...
    if (cloudFiles.entries.isEmpty) {
      ...
      return ServiceResult.success(data: null, message: 'Нет файлов в облаке');
    }
    ...
    if (!isNewer) {
      ...
      return ServiceResult.success(data: null, message: 'Нет новых версий');
    }
  ...
  Future<ServiceResult<String>> downloadArchive({
    ...
    try {
      ...
      return ServiceResult.success(data: downloadPath);
    } catch (e, st) {
  ...
  Future<ServiceResult<String>> replaceDatabase({
    ...
    if (!await file.exists()) {
      return ServiceResult.failure('Скачанный файл не найден: $downloadPath');
    }
    ...
    try {
      ...
      return ServiceResult.success(
        data: importedFolderPath,
        message: 'База данных успешно заменена',
      );
    } catch (e, st) {
  ...
  /// Импорт базы данных из Dropbox (DEPRECATED - использовать отдельные методы)
  ///
  /// [metadata] - метаданные базы данных для синхронизации
  /// [pathToDbFolder] - путь к папке с БД
  /// [encryptionKeyArchive] - опциональный ключ шифрования архива
  @Deprecated(
    'Используйте checkForNewVersion, downloadArchive и replaceDatabase',
  )
